...
function main(arg){
  let options = {
    log: false,
    basePerCount: 10000, // 每次买入基准数量
    basePerValue: 20000, // 每次买入基准数量
    gridStep: 0.02, // 网格
    buyPointRate: 1.6,
    sellPointRate: 1.6,
    ...arg
  ...
  function verifyIsBuyPoint(value, targetValue, upRate){
    if(gridValue * (1 - options.gridStep) >= value){
      gridValue = value;
      if(
        // money >= options.basePerCount * value // 账户有钱
        money >= options.basePerValue // 账户有钱
        && value <= targetValue
        && (buyPoint === null || buyPoint >= value)
      ){
        return true;
      }
    }
  ...
  function verifyIsSellPoint(value, targetValue, upRate){
    if(gridValue * (1 + options.gridStep) <= value){
      gridValue = value;
      if(
        // libCount >= options.basePerCount // 持仓不为空
        libCount >= options.basePerValue / value // 持仓不为空
        && value > targetValue
        && (sellPoint === null || sellPoint <= value)
      ){
        return true;
      }
    }
  ...
  for(let i = 0; i < dayDataList.length; i++){
    ...
    // 判断买入点
    let isBuyPoint = verifyIsBuyPoint(value, targetValue, upRate);
    // 判断卖出点
    let isSellPoint = verifyIsSellPoint(value, targetValue, upRate);
    ...
    if(isSellPoint || isBuyPoint){
      if(isSellPoint){
        ...
        currentCostCount = -options.basePerValue / value;
        sellPoint = value;
        buyPoint = null;
      }else if(isBuyPoint){
        handlerCount.buy ++;
  ...
  // console.log(_.countBy(diffToMeanUpRateList)); // 高于平均高估值的范围
...
function testMore(){
   ...
   * buyPointRate: 1.1,
   sellPointRate: 1.2,
   */
  let area1 = null || Array(10).fill(null).map((item, index)=>{
    return index * 0.1 + 1;
  });
  let area2 = null || Array(12).fill(null).map((item, index)=>{
    return index * 0.1 + 1;
  });
  let nowData = null;
  let nowOption = null;
  let days = 0;
  let i = 0;
  for(let buyPointRate of area1){
    for(let sellPointRate of area2){
      // for(let emptyClearStep of emptyClearStepArea){
      console.log(++i);
      let option = {log: false, buyPointRate, sellPointRate};
