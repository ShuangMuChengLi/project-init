...
function main(arg){
  let options = {
    log: false,
    basePerBuyValue: 20000, // 每次买入价格
    basePerSellValue: 7000, // 每次卖出价格
    buyPointRate: 0.9,
    sellPointRate: 1,
    girdBuyDiffDay: 1,
    girdSellDiffDay: 2,
  ...
  function verifyIsBuyPoint(value, upRate, currentMoment){
    return money >= options.basePerBuyValue + 500000 // 账户有钱
      && upRate < options.buyPointRate
  ...
  for(let i = 0; i < dayDataList.length; i++){
    ...
    let targetValue = base * Math.pow(perGrowAllDay, nowDiffDays) * meanUpRate;
    let upRate = value / targetValue;
    // upRateList.push(upRate);
    // let diffToMeanUpRate = upRate / meanUpRate;
    // 判断买入点
    let isBuyPoint = verifyIsBuyPoint(value, upRate, currentMoment);
    // 判断卖出点
...
function testMore(){
  /**
   ...
   basePerBuyValue: 20000, // 每次买入基准价格
   basePerSellValue: 5000, // 每次卖出基准价格
   buyPointRate: 0.9,
   sellPointRate: 1,
   girdBuyDiffDay: 1,
   girdSellDiffDay: 1,
  ...
  let area4 = null || Array(5).fill(null).map((item, index)=>{
    return index + 1;
  });
  let nowData = null;
  let nowOption = null;
    ...
    for(let areaItem2 of area2){
      for(let areaItem3 of area3){
        for(let areaItem4 of area4) {
          ...
          let option = {
            log: false,
            basePerBuyValue: areaItem1,
            basePerSellValue: areaItem2,
            girdBuyDiffDay: areaItem3,
            girdSellDiffDay: areaItem4
          };
